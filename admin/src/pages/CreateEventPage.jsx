...
const CreateEventPage = () => {
  ...
  // ✅ Handle Rules Change (Convert to Array)
  const handleRulesChange = (e) => {
    setFormData({ ...formData, rules: e.target.value.split(",").map(rule => rule.trim()) });
  };

  // ✅ Handle Social Links Change (Convert to Array)
  const handleSocialLinksChange = (e) => {
    setFormData({ ...formData, socialLinks: e.target.value.split(",").map(link => link.trim()) });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ Form Validations
    if (!formData.title || !formData.description || !formData.category || !formData.subcategory || !formData.startTime || !formData.endTime) {
    ...
    <div className="min-h-screen bg-gray-900 text-white p-6">
      ...
      >
        ...
        <input
          type="text"
          name="subcategory"
          placeholder="Enter Event Subcategory"
        ...
        {/* Price & Free Checkbox */}
        <label className="block mb-2">Price (If Paid)</label>
        <input
          type="number"
          name="price"
